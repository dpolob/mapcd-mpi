...
int main(int argc, char** argv) {
    ...
    if (COMM_CART != MPI_COMM_NULL) {
	    ...
	    srand ( time(NULL) );
            ...
            MPI_Recv(&ImagenLocal[tama_local * coord[0]][tama_local * coord[1]], 1, subImagen, MPI_ANY_SOURCE, 0, COMM_CART, &status);
    ...
    if (der != MPI_PROC_NULL)//si tengo vecino derecha recibo su columna derecha.
        MPI_Recv(&ImagenLocal[tama_local * coord[0]]
            [(tama_local * coord[1]) + tama_local], 1, vectorVertical, der, 0, COMM_CART, &status);

    if (der != MPI_PROC_NULL)//si tengo vecino derecha mando mi columna derecha
        MPI_Send(&ImagenLocal[tama_local * coord[0]]
            [(tama_local * coord[1]) + tama_local - 1], 1, vectorVertical, der, 0, COMM_CART);
    ...
    for (int i = 0; i < tama_local; i++) {
        for (int j = 0; j < tama_local; j++) {
            int nuevo = ImagenLocal[tama_local * coord[0] + i][tama_local * coord[1] + j]*2;
            int div = 2;
            if (coord[0] != 0) {//Arriba
                nuevo += ImagenLocal[tama_local * coord[0] + i - 1][tama_local * coord[1] + j];
                div++;
            }
            ...
            if (coord[1] != 0) {//Izquierda
                nuevo += ImagenLocal[tama_local * coord[0] + i ][tama_local * coord[1] + j - 1];
                div++;
            }
    ...
    if (rank != 0) {//Si no soy el proceso 0 mando mi matriz
        ...
        for (int i = 0; i < tama_local; i++)
            for (int j = 0; j < tama_local; j++)
                ImagenLocal[(tama_local * coord[0]) + i][(tama_local * coord[1]) + j] = local[i][j];
        for (int i = 0; i < size - 1; i++) {
            MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, COMM_CART, &status);
            MPI_Cart_coords(COMM_CART, status.MPI_SOURCE, 2, coord);
            MPI_Recv(&ImagenLocal[tama_local * coord[0]]
                    [tama_local * coord[1]], 1, subImagen, status.MPI_SOURCE, 0, COMM_CART, &status);
